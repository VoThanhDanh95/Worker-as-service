...
# using WKRClient in multicast way
...
def client_clone(id, idx):
    bc = WKRClient(port=int(sys.argv[1]), port_out=int(sys.argv[2]), identity=id)
    for j in bc.fetch():
        print('clone-client-%d: received %d x %d' % (idx, j.shape[0], j.shape[1]))


if __name__ == '__main__':
    bc = WKRClient(port=int(sys.argv[1]), port_out=int(sys.argv[2]))
