...
No. Think of ``WKRClient`` as a general feature extractor, whose
...
setting up ``WKRServer`` is just a one-time thing, which can be even
run in a docker container. To ease the integration, we support python
2 on the client side so that you can directly use ``WKRClient`` as a
...
I encounter ``zmq.error.ZMQError: Operation cannot be accomplished in current state`` when using ``WKRClient``, what should I do?
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

This is often due to the misuse of ``WKRClient`` in
multi-thread/process environment. Note that you can’t reuse one
``WKRClient`` among multiple threads/processes, you have to make a
